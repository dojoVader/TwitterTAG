import {
  ...
  dom,
  q,
  TWITTER_COLLECTION_PROFILE,
  TWITTER_DROPDOWN_NODE,
  TWITTER_LD_JSON,
  TWITTER_PRIMARY_COLUMN,
  TWITTER_ROOT_NODE,
  TWITTER_SELECTOR,
  TWITTER_USER_ACTION,
  TWITTER_USER_NAME_NODE,
  TwitterTagData,
} from "../utils/helpers";
...
export class TwitterHandle {
    ...
    if (findElement) {
      ...
      if (hasTimeLine) {
        ...
        const tweets = q(
          TWITTER_COLLECTION_PROFILE,
          true,
          q(TWITTER_PRIMARY_COLUMN) as HTMLElement,
  ...
  renderLabel(tweet: HTMLElement, users: TwitterTagData[]) {
    ...
    if (userName) {
      ...
      root.render(
        <TwitterBadgePill
          color={foundName?.color}
          label={foundName ? foundName?.label : ""}
        />,
      );
