...
export function generateName(length: number){
    ...
    while(counter <= length){
        letter+=alphabet.charAt(Math.floor(Math.random() * characterLength)).toString();
     counter++;
    }
...
export async function generateEmailForInput(): Promise<string>{
    const currentEmail = await chrome.storage.local.get(configuration.SCHEMA.CURRENT_EMAIL);
    if(currentEmail.hasOwnProperty(configuration.SCHEMA.CURRENT_EMAIL)) {
        return currentEmail[configuration.SCHEMA.CURRENT_EMAIL];
    }else{
        return "";
    }
}
